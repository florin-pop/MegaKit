...
class FileHandleMock: SequentialIOHandle {
    ...
    func readData(ofLength: Int) -> Data {
        let dataToRead = Self.dataToRead[readURL]!
        let length = min(dataToRead.count, ofLength)
        if length == 0 {
            return Data()
        }
        let data = dataToRead[..<length]
        Self.dataToRead[readURL]! = Data(dataToRead[length...])
        return data
    }

    func write(_ data: Data) {
        var writtenData = Self.dataWritten[writeURL] ?? Data()
        writtenData.append(data)
        Self.dataWritten[writeURL] = writtenData
    }
}
...
class FailingInitForReadingFileHandleMock: FileHandleMock {
    required init(forReadingFrom _: URL) throws {
...
class FailingInitForWritingFileHandleMock: FileHandleMock {
    ...
    required init(forWritingTo _: URL) throws {
...
class AESDecryptorTests: XCTestCase {
    ...
    func testDecryptJpeg() throws {
        ...
        XCTAssertEqual(totalBytesWritten, 254_082)
