...
public struct MegaClient {
    ...
    public func login(using email: String, password: String, completion: @escaping (Result<String, MegaError>) -> Void) {
        ...
        do {
            request = try URLRequest(sequence: sequence, payload: requestPayload, queryItems: [])
        } catch let error {
            completion(.failure(error as! MegaError))
            return
        }
    ...
    func openSession(email: String, userHash: String, completion: @escaping (Result<Data, MegaError>) -> Void) {
        ...
        urlComponents?.queryItems = [
            URLQueryItem(name: "id", value: "\(self.sequence.next())"),
        ]
        ...
        guard let requestData = try? JSONSerialization.data(withJSONObject: requestPayload, options: [.sortedKeys]) else {
            completion(.failure(.requestFailed))
            return
        }
...
//            if let p2key = "secret".data(using: .utf8),
//               let cipher = AES(key: p2key, blockMode: .cbc, padding: .noPadding),
//               encmkey =
